 ...
 *
 * Adapted from GitNexus's `language-config.ts#loadTsconfigPaths` but:
 *   - synchronous (we read at factory init time)
 *   - follows `extends` chains
 *   - returns a small object with a `rewrite()` helper
 */
 ...
 * when child does not override them.
 */
function loadAndMerge(
  filePath: string,
  seen = new Set<string>()
): { baseUrl: string | undefined; paths: Record<string, string[]>; configDir: string } | null {
  ...
  let inherited: { baseUrl: string | undefined; paths: Record<string, string[]>; configDir: string } | null = null;
  if (cfg.extends) {
    let extPath = cfg.extends;
    if (!extPath.endsWith('.json')) extPath += '.json';
    const extResolved = isAbsolute(extPath) ? extPath : resolve(configDir, extPath);
    inherited = loadAndMerge(extResolved, seen);
  }

  const co = cfg.compilerOptions ?? {};
  const baseUrl = co.baseUrl ?? inherited?.baseUrl;
  const paths: Record<string, string[]> = { ...(inherited?.paths ?? {}), ...(co.paths ?? {}) };

  // baseUrl is resolved relative to the config that *defined* it.
  return {
    baseUrl,
    paths,
...
export function loadTsconfigAliases(repoPath: string): TsconfigAliasMap {
  ...
  for (const filename of candidates) {
    ...
    // baseUrl is interpreted relative to the config dir that defined it,
    // then re-expressed relative to the repo root for our consumers.
    ...
    for (const [pattern, targets] of Object.entries(merged.paths)) {
      ...
      const prefix = isGlob ? pattern.slice(0, -1) : pattern; // keeps trailing "/"
      const targetExact = repoRel(rawTarget.endsWith('/*') ? rawTarget.slice(0, -2) : rawTarget);
      const targetPrefix = repoRel(rawTarget.endsWith('/*') ? rawTarget.slice(0, -1) : rawTarget) +
        (rawTarget.endsWith('/*') ? '/' : '');
